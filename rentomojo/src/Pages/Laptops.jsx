...
const Laptops = () => {
  ...
  const { sliderValue,isLoading,setIsLoading } = useContext(FilterContext);

  useEffect(() => {
    setIsLoading(true)
    fetch("https://graceful-panama-hat-ray.cyclic.app/electronics/laptops")
      .then((res) => res.json())
    ...
    <div>
      ...
      <div className={styles.smartphones_container}>
        {laptopsData.map((item) => (
          <Link to={`/electronics/laptops/${item.id}`}>
